...
class MatchGame extends Component {
  ...
  resetGame = () => {
    this.setState({...defaultValues}, this.setTimer())
  }
  ...
  renderScoreCard = () => {
    ...
    return (
      <div className="score-card-container">
        ...
        <p className="total-score-value">{score}</p>
  ...
  renderThumbnails = () => {
    ...
    const activeCategoryImages = imagesList.filter(
      eachImg => eachImg.category === activeTabId,
    )
    return (
      <ul className="thumbnails-container">
        {activeCategoryImages.map(eachThumbnail => (
          <ThumbnailItem
  ...
  renderRandomImage = () => {
    const {randomImageId} = this.state
    const randomDetails = imagesList.find(
      eachDetail => eachDetail.id === randomImageId,
    )
    const {imageUrl} = randomDetails
    return (
      <div className="random-image-container">
        <img className="random-image" alt="match" src={imageUrl} />
      </div>
    )
  }
    ...
    return (
      <div className="app-container">
        ...
        <div className="game-background-container">
          ...
          {!isGameActive && this.renderScoreCard()}
        </div>
      </div>
    )
  }
